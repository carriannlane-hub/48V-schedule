...
export default function GamiCon48VSchedule() {
  ...
  const [calendarMenuOpen, setCalendarMenuOpen] = useState(null); // tracks which session's menu is open: "block-session"
        ...
        <div className="px-6 py-5">
          <div className="flex flex-col md:flex-row md:items-start gap-4">
            <div className="flex items-center gap-3 md:block md:flex-shrink-0 md:w-28">
              ...
              {session.additionalType && (() => { const b2 = getTypeBadge(session.additionalType); return (
                <span className={`md:hidden px-2 py-0.5 ${b2.bg} ${b2.text} text-xs font-semibold rounded-full`}>{b2.label}</span>
              ); })()}
            </div>
            <div className="flex-grow">
              <div className="hidden md:flex flex-wrap items-center gap-2 mb-2">
                ...
                {session.additionalType && (() => { const b2 = getTypeBadge(session.additionalType); return (
                  <span className={`px-3 py-1 ${b2.bg} ${b2.text} text-xs font-semibold rounded-full`}>{b2.label}</span>
                ); })()}
                {session.country && (
              ...
              <div className="mt-3 relative calendar-dropdown">
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                    const menuId = `highlight-${block.block}-${sessionIndex}`;
                  ...
                  aria-expanded={calendarMenuOpen === `highlight-${block.block}-${sessionIndex}`}
                  aria-haspopup="true"
                ...
                >
                  ...
                  Add to Calendar
                  <svg className={`w-4 h-4 transition-transform ${calendarMenuOpen === `highlight-${block.block}-${sessionIndex}` ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
                  </svg>
                </button>
                {calendarMenuOpen === `highlight-${block.block}-${sessionIndex}` && (
                  <div className="absolute left-0 mt-2 w-56 bg-slate-800 border border-slate-600 rounded-lg shadow-xl z-50" role="menu">
                    <a
                      href={getGoogleCalendarUrl(session, sessionIndex, block)}
                      target="_blank"
                    ...
                    <button
                      onClick={() => {
                        generateICS(session, sessionIndex, block);
                        setCalendarMenuOpen(null);
                      }}
      ...
      <section className="relative z-10 px-4 py-12 sm:py-16 text-center">
        <div className="max-w-4xl mx-auto">
          <p className="text-amber-400 uppercase tracking-widest text-xs sm:text-sm mb-4" style={{ fontFamily: 'Josefin Sans, sans-serif' }}>
            {getLocalEventDates()} <span className="normal-case">(your time)</span> • Live Online
          </p>
                          ...
                          <div className="flex-grow">
                            ...
                            <div className="mt-3 relative calendar-dropdown">
                              <button
                                ...
                                className="w-full sm:w-auto inline-flex items-center justify-center sm:justify-start gap-2 px-4 py-3 sm:py-2.5 text-sm text-slate-300 bg-slate-700/50 hover:bg-slate-600 active:bg-slate-500 rounded-lg transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-400 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-800 min-h-[44px]"
                                aria-expanded={calendarMenuOpen === `block-${block.block}-${idx}`}
                                aria-haspopup="true"
                                aria-label={`Add ${session.title} to calendar`}
                              >
                                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                                ...
                                Add to Calendar
                                <svg className={`w-4 h-4 transition-transform ${calendarMenuOpen === `block-${block.block}-${idx}` ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
                                </svg>
                              </button>
                              {calendarMenuOpen === `block-${block.block}-${idx}` && (
                                <div className="absolute left-0 mt-2 w-56 bg-slate-800 border border-slate-600 rounded-lg shadow-xl z-50" role="menu">
                                  <a
